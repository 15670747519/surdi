	...
	function add(){
		if ($this->syArgs('run')==1){
			...
			if(false == $newVerifier){
				...
				if($thenewfields['fieldstype']!="select" && $thenewfields['fieldstype']!="radio" && $thenewfields['fieldstype']!="checkbox"){$thenewfields['selects']='';}
	...
	function edit(){
		...
		if ($this->syArgs('run')==1){
			...
			if(false == $newVerifier){
				...
				if($thenewfields['fieldstype']!="varchar"){$thenewfields['fieldslong']=0;}
				if($thenewfields['fieldstype']!="select" && $thenewfields['fieldstype']!="checkbox" && $thenewfields['fieldstype']!="radio"){$thenewfields['selects']='';}
		...
		foreach($v as $f){
			switch ($f['fieldstype']){
				...
				case 'radio':
					...
					foreach(explode(',',$f['selects']) as $v){
						$s=explode('=',$v);
						$t.='<label><input type="radio" id="'.$f['fields'].'" name="'.$f['fields'].'" value="'.$s[1].'" ';
						if($c[$f['fields']]==$s[1])$t.='checked="checked"';
						$t.='>'.$s[0].'</label>';
					}
				break;
				case 'checkbox':
