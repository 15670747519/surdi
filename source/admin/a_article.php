	...
	function __construct(){
		...
		if(($this->syArgs('a',1)=='add'||$this->syArgs('a',1)=='edit')&&$this->syArgs('run')==1){
			...
			foreach($v as $f){
				...
				if($f['fieldstype']=='text'){$ns=code_body($this->syArgs($f['fields'],2));}
					...
					if($edit_field){
						...
						if($this->syArgs('tui')){
							...
							foreach ($tids as $key => $tid) {
								if(!$this->auser->checkclass($tid)){continue;}
								$newrow3 = $this->newrow1;
								$newrow3['tid'] = $tid;
								$newrow3['user'] = $this->carray['user'];
								$newrow3['usertype'] = $this->carray['usertype'];
								$newVerifier=$this->ClassADD->syVerifier($newrow3);
								if(false == $newVerifier){
									$addnewrow1=$this->ClassADD->create($newrow3);
									if($addnewrow1==FALSE){message_a("主表数据写入失败，请重新提交");}
									$arrays = array(
										'aid' => $addnewrow1,
									);
									...
									syDB($this->molds.'_field')->runSql('set interactive_timeout=24*3600');
									if(syDB($this->molds.'_field')->create($this->newrow2)){
										if($GLOBALS['G_DY']['rewrite']["rewrite_open"]!=1&&syExt('site_html')==1&&$newrow3['isshow']==1&&$newrow3['mrank']==0&&$newrow3['mgold']==0){
											$c_html_f=html_rules($this->molds,$tid,$newrow3['addtime'],$addnewrow1,$newrow3['htmlfile']);
											syDB($this->molds)->updateField(array('id'=>$addnewrow1),'htmlurl',$c_html_f);
											$this->chtml->c_molds('article',array('id'=>$addnewrow1),$c_html_f);
											$body=array_filter(explode("[doyo|page]",$this->newrow2['body']));
											$allb=count($body);
