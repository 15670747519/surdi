...
class a_member extends syController
{
		...
		if ($this->syArgs('run')==1){
			...
			$newVerifier=$this->cu->syVerifier($this->newrow1);
				if(false == $newVerifier){
					//Ucenter 同步
					$uid = syClass("syUcenter")->uc_user_register($this->syArgs('user',1), $this->syArgs("pass",1), $this->syArgs('email',1));
					...
					$this->newrow1 = array_merge($this->newrow1,array('uid'=>$uid));
	...
	function edit(){
		$m = $this->d=$this->cu->find(array('id'=>$this->syArgs('id')));
		if ($this->syArgs('run')==1){
			$a=$this->newrow1;
			if($this->syArgs('pass',1)==''){unset($a['pass']);}
				//Ucenter
				$ucresult = syClass("syUcenter")->uc_user_edit($m['user'], '', $this->syArgs('pass'), $this->syArgs('email'), 1);
				if($ucresult != 1) {
					message('修改失败');
				} 
