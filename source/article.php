...
class article extends syController
{
	...
	function index(){
		...
		foreach($this->sy_class_type->navi($this->article['tid']) as $k => $v){
			...
			if($k == 0){
				$this->positions.='<span class="mainc"><a href="'.html_url('classtype',$d_pos).'">'.$v['classname'].'</a></span>';
			}else{
				$this->positions.='<span class="subc"><a href="'.html_url('classtype',$d_pos).'">/'.$v['classname'].'</a></span>';
			}
		}
	...
	function type(){
		...
		if($this->type['isindex']==1||$this->type['isindex']==2||$this->type['isindex']==0){
			$w.=" where isshow=1 ";
	...
	function score(){
		if($this->syArgs('id')){
			$people = syDB('comment')->findSql('select count(cid) as ct from '.$GLOBALS["G_DY"]["db"]["prefix"].'comment where aid='.$this->syArgs('id').' and score is not null');
			$all = syDB('comment')->findSql('select SUM(score) as ct from '.$GLOBALS["G_DY"]["db"]["prefix"].'comment where aid='.$this->syArgs('id').' and score is not null');
			$hits=round($all[0]['ct']/$people[0]['ct'],2);
	...
	function search(){
		...
		$this->total_page = $total_page=total_page($this->db.$w);
		...
		$this->lists=$list_c;
		$this->positions='<span class="mainc">内容搜索</span>';
