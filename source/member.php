...
{	
		...
		if($this->syArgs("go")==1){
			if($this->syArgs("user",1) && $this->syArgs("pass",1)){
				...
				if(!$r){
					...
					//ucenter 同步登录
					list($uid, $username, $password, $email) = syClass("syUcenter")->uc_user_login($this->syArgs('user',1), $this->syArgs('pass',1));
		...
		if($id&&$token){
			...
			if($t>259200){
				...
				if($this->syArgs("go")){
					...
					//Ucenter
					$ucresult = syClass("syUcenter")->uc_user_edit($m['user'], '', $this->syArgs('pass1'), '', 1);
					if($ucresult != 1) {
						message('找回密码参数有误');
					} 
	...
	function out(){
		...
		session_destroy();
		syClass("syUcenter")->uc_user_synlogout();
		jump($GLOBALS['WWW']);
	}
	function rules(){
	...
	function reg(){
		...
		if($this->syArgs("go")==1){
			...
			if(syDB('member')->find(array('user'=>$this->syArgs('user',1))) || (syClass("syUcenter")->uc_user_checkname($this->syArgs('user',1)) == '-3'))message("用户名已被注册，请更换");
			if(!$this->syArgs('pass1',1))message("请输入密码");
			if(!$this->syArgs('pass2',1))message("请输入确认密码");
			...
			//Ucenter 同步
			$uid = syClass("syUcenter")->uc_user_register($this->syArgs('user',1), $this->syArgs("pass1",1), $this->syArgs('email',1));
			...
			if(false == $newVerifier){
				...
				//ucenter 同步登录
				list($uid, $username, $password, $email) = syClass("syUcenter")->uc_user_login($this->syArgs('user',1), $this->syArgs('pass1',1));
				if($uid < 1){
					message("注册失败，请重新注册");
				}
	...
	function myinfo(){
		if($this->syArgs("go")==1){
			...
			if($this->syArgs('pass',1)||$this->syArgs('pass1',1)||$this->syArgs('pass2',1)){
				...
				if($this->syArgs('pass1',1)!=$this->syArgs('pass2',1))message("两次密码输入不一致");
				$ucresult = syClass("syUcenter")->uc_user_edit($this->my['user'], $this->syArgs('pass',1), $this->syArgs('pass1'));
				if($ucresult != 1) {
					message('原密码错误');
				} 
	...
	function release(){
		...
		switch ($this->type['molds']){
			case 'article':
				$a=array(
					array('name'=>'标题','input'=>'<input name="title" id="title" type="text" class="inp form-input" value="'.$c['title'].'">','fields'=>'title'),
					array('name'=>'内容','input'=>'<script type="text/javascript">$(function(){KindEditor.create("#body",{cssPath : ["include/js/prettify.css"],fileManagerJson : "/index.php?c=uploads&a=filemanager",allowFileManager : true,filePostName : "editor_KindEditor",filterMode : false,uploadJson : "/index.php?c=uploads&tid='.$tid.'&isfiles=editor_KindEditor"})});</script><textarea name="body" id="body" class="inp" style="width:550px;height:300px;">'.$c['body'].'</textarea>','fields'=>'body'),
				);
	...
	private function fields_args($molds,$tid=0,$lists=0){
		...
		foreach($v as $f){
			$ns='';$n=array();
			if($f['fieldstype']=='varchar' || $f['fieldstype']=='files' || $f['fieldstype']=='fileall' || $f['fieldstype']=='select' || $f['fieldstype']=='radio' || $f['fieldstype']=='text'){$ns=$this->syArgs($f['fields'],1);}
