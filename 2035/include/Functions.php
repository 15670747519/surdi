...
//获取最外层栏目id
function typepid($tid){
	$pid['pid'] = $tid;
    while($pid['pid'] != 0){
    	$tid = $pid['pid'];
        $pid = syDB('classtype')->find(array('tid' => $tid),null,'pid');
    }
	return $tid;
}
//自定义标签
...
function pagetxt($pagearray,$pageno=3,$vp='page'){
	$pagetxt='';
	if($pagearray['total_count']>0)$pagetxt.='<li class="counter wider"><a>共'.$pagearray['total_count'].'篇</a></li>';
	$pageurl=$_SERVER["QUERY_STRING"];
	if($pagearray['current_page']>1){
		$pagetxt.='<li class="first"><a href="'.url_set_value($pageurl,$vp,1).'">首页</a></li><li class="wider"><a href="'.url_set_value($pageurl,$vp,$pagearray['prev_page']).'">上一页</a></li>';
	}
	...
	if($pagearray['current_page'] < $pagearray['last_page']){
		$pagetxt.='<li class="wider"><a href="'.url_set_value($pageurl,$vp,$pagearray['next_page']).'">下一页</a></li><li class="last"><a href="'.url_set_value($pageurl,$vp,$pagearray['last_page']).'">尾页</a></li>';
	}
	return $pagetxt;
}
//静态html分页代码
...
function pagetxt_ajax($pagearray,$url,$ajax,$pageno=3){
	...
	if($pagearray['total_count']>0)$pagetxt.='<li><a>共'.$pagearray['total_count'].'条</a></li>';
	if($pagearray['current_page']>1){
		$pagetxt.='<li class="first"><a onClick="'.str_replace('[_page_]',1,$ajax).'">首页</a></li><li><a onClick="'.str_replace('[_page_]',$pagearray['prev_page'],$ajax).'">上一页</a></li>';
	}
	$pageno1=$pagearray['current_page']-$pageno;if($pageno1<1){$pageno1=1;}
	$pageno2=$pagearray['current_page']+$pageno;if($pageno2>$pagearray['total_page']){$pageno2=$pagearray['total_page'];}
	while($pageno1<=$pageno2){
	...
	if($pagearray['current_page'] < $pagearray['last_page']){
		$pagetxt.='<li><a onClick="'.str_replace('[_page_]',$pagearray['next_page'],$ajax).'">下一页</a></li><li class="last"><a onClick="'.str_replace('[_page_]',$pagearray['last_page'],$ajax).'">尾页</a></li>';
	}
	return $pagetxt;
}
...
function getimg($path,$size,$type){
	$path = explode('.',$path);
	$file = $path[0] . '_' . $size . '.' . $path[1];
	$url = '/2035/getimg.php?img=' . $file . '&gen=' . $type;
	return $url;
}