...
class a_message extends syController
{
	...
	function export(){
		$bgtime = strtotime($this->syArgs('bgdate',1));
		$edtime = strtotime($this->syArgs('eddate',1));
		$conditions=' where 1=1';
		if($bgtime){
			$conditions.=' and addtime >= '.$bgtime;
		}
		if($endtime){
			$conditions.=' and addtime <= '.$edtime;
		}
		$sql='select * from '.$this->sqldb.$conditions.' order by orders desc,addtime desc,id desc';
		$result = $this->ClassADD->findSql($sql); 
		header( "Cache-Control: public" );
	    header( "Pragma: public" );
		header("Content-type:application/vnd.ms-excel");
	    header("Content-Disposition:attachment;filename=留言.csv");
	    header('Content-Type:APPLICATION/OCTET-STREAM');
		ob_start();
		$header_str =  iconv("utf-8",'gbk',"姓名,标题,内容\n");
		$file_str="";
		...
		if($result){
			foreach ($result as $row) {
				...
				$user = iconv('utf-8','gb2312',$row['user']); //中文转码 
        		$title = iconv('utf-8','gb2312',$row['title']); 
        		$body = iconv('utf-8','gb2312',$body); 
        		$addtime = iconv('utf-8','gb2312',date('Y-m-d H:i:s', $row['addtime'])); 
				$file_str.= $user.','.$title.','.$body.','.$addtime."\n";
			}
		}else{
			echo "还没有任何留言！";
		}
