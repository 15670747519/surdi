<?php
if (!defined('APP_PATH') || !defined('DOYO_PATH')) {
    exit('Access Denied');
}
class a_article extends syController
{
    function __construct()
    {
        ...
        if (($this->syArgs('a', 1) == 'add' || $this->syArgs('a', 1) == 'edit') && $this->syArgs('run') == 1) {
            ...
            if ($this->syArgs('trait', 2)) {
                $trait = ',' . implode(',', $this->syArgs('trait', 2)) . ',';
            } else {
                $trait = '';
            }
            if ($this->syArgs('tid')) {
            ...
            if ($this->syArgs('a', 1) == 'add') {
                $this->newrow1 = array_merge($this->newrow1, array(
                    'user' => $this->auserinfo['auser']
                ));
            }
            $this->newrow2 = array(
                'body' => code_body($this->syArgs('body', 2))
            );
            $v             = $this->ClassF->findAll(" molds='" . $this->molds . "' and types like '%|" . $this->syArgs('tid') . "|%' ");
            foreach ($v as $f) {
                $ns = '';
                $n  = array();
                if ($f['fieldstype'] == 'varchar' || $f['fieldstype'] == 'files' || $f['fieldstype'] == 'select') {
                    $ns = $this->syArgs($f['fields'], 1);
                }
                if ($f['fieldstype'] == 'int') {
                    $ns = $this->syArgs($f['fields']);
                }
                if ($f['fieldstype'] == 'contingency' && $this->syArgs('contingency_' . $f['fields'] . '_word', 1)) {
                    $ns = $this->syArgs($f['fields']);
                }
                if ($f['fieldstype'] == 'decimal') {
                    $ns = $this->syArgs($f['fields'], 3);
                }
                if ($f['fieldstype'] == 'text') {
                    $ns = code_body($this->syArgs($f['fields'], 4));
                }
                if ($f['fieldstype'] == 'time') {
                    $ns = strtotime($this->syArgs($f['fields'], 1));
                }
                if ($f['fieldstype'] == 'fileall') {
                    $fieldsall = $this->syArgs($f['fields'] . 'file', 2);
                    if ($fieldsall) {
                        $num = $this->syArgs($f['fields'] . 'num', 2);
                        $txt = $this->syArgs($f['fields'] . 'txt', 2);
                        $ns  = '';
                        natsort($num);
                        foreach ($num as $k => $v) {
                ...
                if ($f['fieldstype'] == 'checkbox') {
                    if ($this->syArgs($f['fields'], 2)) {
                        $ns = '|' . implode('|', $this->syArgs($f['fields'], 2)) . '|';
                    } else {
                        $ns = '';
                    }
                }
    ...
    function index()
    {
        ...
        if ($this->auserinfo['level'] == 1) {
            if ($this->syArgs("tid")) {
                $conditions .= "and tid in(" . $this->types->leafid($this->syArgs('tid')) . ") ";
            }
        } else {
            if ($this->syArgs("tid") && $this->auser->checkclass($this->syArgs("tid"))) {
                $conditions .= "and tid in(" . $this->types->leafid($this->syArgs('tid')) . ") and tid in(" . trim($this->auserinfo['pclasstype'], ',') . ") ";
            } else {
                if ($this->auser->checkclass() == false)
                    message_a("您没有栏目权限");
                $conditions .= "and tid in(" . trim($this->auserinfo['pclasstype'], ',') . ") ";
            }
        }
        ...
        if ($this->syArgs("sid")) {
            $conditions .= "and sid=" . $this->syArgs('sid') . " ";
        }
        if ($this->syArgs("trait")) {
            $conditions .= "and trait like '%," . $this->syArgs('trait') . ",%' ";
        }
        if ($this->syArgs("litpic")) {
            if ($this->syArgs("litpic") == 1) {
                $conditions .= "and litpic!='' ";
            } else {
                $conditions .= "and litpic='' ";
            }
        }
        if ($this->syArgs("title", 1)) {
            $conditions .= "and title like '%" . $this->syArgs('title', 1) . "%' ";
        }
        if ($conditions != '') {
            $conditions = ' where ' . substr($conditions, 3);
        }
    ...
    function add()
    {
        ...
        if ($this->syArgs('run') == 1) {
            if (!$this->auser->checkclass($this->newrow1['tid'])) {
                message_a("无权操作本栏目内容");
            }
            ...
            if (false == $newVerifier) {
                ...
                if ($this->syArgs('all_down_images') == 1) {
                    ...
                    foreach ($pic as $v) {
                        ...
                        if ($v) {
                            ...
                            if ($localUrl) {
                                $bodynew = str_ireplace($v, $GLOBALS['WWW'] . $localUrl, $bodynew);
                            }
                        }
                    }
                    $arrays = array_merge($arrays, array(
                        'body' => addslashes($bodynew)
                    ));
                }
                ...
                if (syDB($this->molds . '_field')->create($this->newrow2)) {
                    if ($GLOBALS['G_DY']['rewrite']["rewrite_open"] != 1 && syExt('site_html') == 1 && $this->newrow1['isshow'] == 1 && $this->newrow1['mrank'] == 0 && $this->newrow1['mgold'] == 0) {
                        ...
                        syDB($this->molds)->updateField(array(
                            'id' => $addnewrow1
                        ), 'htmlurl', $c_html_f);
                        $this->chtml->c_molds('article', array(
                            'id' => $addnewrow1
                        ), $c_html_f);
                            ...
                            for ($i = 1; $i <= $allb; $i++) {
                                if ($i > 1) {
                                    $this->chtml->c_molds('article', array(
                                        'id' => $addnewrow1,
                                        'page' => $i
                                    ), str_replace('.', $i . '.', $c_html_f));
                                }
                            }
                        }
                    }
                    deleteDir($GLOBALS['G_DY']['sp_cache']);
                    message_a($this->moldname . '添加成功', '?c=a_' . $this->molds, '<a href="?c=a_' . $this->molds . '&tid='. $this->itid .'">返回列表</a><a href="?c=a_' . $this->molds . '&a=add&tid=' . $this->newrow1['tid'] . '">继续添加</a>', "8");
                } else {
                    syDB($this->molds)->delete(array(
                        'id' => $addnewrow1
                    ));
                    message_a("附表数据写入失败，请重新提交");
                }
            } else {
                message_b($newVerifier);
            }
        }
    ...
    function edit()
    {
        ...
        if (!$this->auser->checkclass($this->carray['tid'])) {
            message_a("无权操作本栏目内容");
        }
        if ($this->syArgs('run') == 1) {
            if ($this->auserinfo['oneself'] && $this->carray['auser'] != $this->auserinfo['auser'] && $this->carray['usertype'] != 0)
                message_a("无权操作本内容");
            $newVerifier = $this->ClassADD->syVerifier($this->newrow1);
            if (false == $newVerifier) {
                if ($this->ClassADD->update(array(
                    'id' => $this->syArgs('id')
                ), $this->newrow1) == FALSE) {
                ...
                if ($this->syArgs('all_down_images') == 1) {
                    ...
                    $this->newrow2 = array_merge($this->newrow2, array(
                        'body' => addslashes($bodynew)
                    ));
                }
                syDB($this->molds . '_field')->runSql('set interactive_timeout=24*3600');
                if (!syDB($this->molds . '_field')->find(array(
                    'aid' => $this->syArgs('id')
                ))) {
                    $this->newrow2 = array_merge($this->newrow2, array(
                        'aid' => $this->syArgs('id')
                    ...
                    $edit_field = syDB($this->molds . '_field')->update(array(
                        'aid' => $this->syArgs('id')
                    ), $this->newrow2);
                }
                if ($edit_field) {
                    ...
                    message_a("附表数据修改失败，请重新提交");
                }
            } else {
                message_b($newVerifier);
            }
        }
        $this->traits = syDB('traits')->findAll(array(
            'molds' => $this->molds
        ));
    ...
    function del()
    {
        ...
        $this->del    = $this->ClassADD->find(array(
            'id' => $this->syArgs('id')
        ));
        if (!$this->auser->checkclass($this->del['tid'])) {
            message_a("无权操作本栏目内容");
        }
        $id = $this->del['id'];
        if ($this->syArgs('run') == 1) {
            if ($this->auserinfo['oneself'] && $this->carray['auser'] != $this->auserinfo['auser'] && $this->carray['usertype'] != 0)
                message_a("无权操作本内容");
            if (syDB($this->molds)->delete(array(
                'id' => $id
            )) && syDB($this->molds . '_field')->delete(array(
                'aid' => $id
            ))) {
                deleteDir($GLOBALS['G_DY']['sp_cache']);
                message_a("删除成功", "?c=a_" . $this->molds);
            } else {
                message_a("删除失败,请重新提交");
            }
        }
    ...
    function alledit()
    {
        ...
        switch ($this->syArgs('go')) {
            case 0:
                foreach ($ids as $tp) {
                    $a = $this->ClassADD->find(array(
                        'id' => $tp
                    ), null, " id,tid ");
                    if ($this->auser->checkclass($a['tid']))
                        $this->ClassADD->update(array(
                            'id' => $tp
                        ), array(
                            'isshow' => 1
                        ));
