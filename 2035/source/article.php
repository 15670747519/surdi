...
class article extends syController
{
	...
	function index(){
		...
		$this->type=syDB('classtype')->find(" molds='article' and tid=".$this->article['tid']." ",null,'tid,pid,classname,litpic,keywords,description,t_content,htmldir,htmlfile,mrank,msubmit');
		...
		$this->positions='';
		foreach($this->sy_class_type->navi($this->article['tid']) as $k => $v){
			$d_pos=syDB('classtype')->find(array('tid'=>$v['tid']),null,'tid,molds,gourl,htmldir,htmlfile,mrank');
			if($k == 0){
				$this->positions.='<a href="'.html_url('classtype',$d_pos).'">'.$v['classname'].'</a>';
			}else{
				$this->positions.='<a href="'.html_url('classtype',$d_pos).'"><small>&nbsp;/&nbsp;'.$v['classname'].'</small></a>';
			}
		}
		$this->back = html_url('classtype',syDB('classtype')->find(array('tid'=>$this->article['tid']),null,'tid,molds,htmldir,htmlfile,mrank'));
	...
	function type(){
		...
		if($this->type['isindex']==1||$this->type['isindex']==2){
			...
			if($tid==33){
				$order=' order by orders desc,title asc';
			}else{
		...
		// 	$this->positions.='<a href="'.html_url('classtype',$d_pos).'">'.$v['classname'].'</a>';
		// }
		foreach($type_pos as $k => $v){
			$d_pos=syDB('classtype')->find(array('tid'=>$v['tid']),null,'tid,gourl,molds,htmldir,htmlfile,mrank');
			if($k == 0){
				$this->positions.='<a href="'.html_url('classtype',$d_pos).'">'.$v['classname'].'</a>';
			}else{
				$this->positions.='<a href="'.html_url('classtype',$d_pos).'"><small>&nbsp;/&nbsp;'.$v['classname'].'</small></a>';
	...
	function search(){
		...
		$w.=" where isshow=1 ";
		$this->word = $word = htmlspecialchars($this->syArgs('word',1));
		if($word){
			$w.=" and (";
			$str = explode(' ',$word);
			foreach($str as $s){
		...
		$this->total_sum = $total_page=total_page($this->db.$w);
		...
		$this->lists=$list_c;
		$this->positions='<a href="'.$GLOBALS["WWW"].'">首页</a>  <small>&nbsp;/&nbsp;搜索“'.$this->syArgs('word',1).'”</small>';
		$this->display('article/search.html');
	}

	function rss(){
		include_once(DOYO_PATH."/ext/syRSS.php");//引入RSS PHP类
		$RSS = new RSS($GLOBALS['S']['title'], $GLOBALS['S']['http'], $GLOBALS['S']['description']);
		$sql = 'select id,tid,title,addtime,description from '.$this->db.' order by addtime DESC limit 20';
		$lists = $this->Class->findSql($sql);
		foreach ($lists as $key => $value) {
			$url = html_url('article', $value);
			$RSS->AddItem($value['title'], $url, $value['description'], date('Y-m-d H:i:s', $value['addtime']));
		}
		$RSS->Show();
	}
}	