...
class ajax extends syController
{
	...
	function online(){
		...
		if($onlineip){
			...
			if($session->find($aIp)){
				$session->updateField($aIp, 'lastactivity', time());
			}else{
				$newArr = array('lastactivity' => time(),'sid' => random(6));
				$newArr = array_merge($newArr, $aIp);
				$session->create($newArr);
				syDB('sysconfig')->incrField(array('name'=>'totalnum'), 'sets');
			}
		}
		syDB('sessions')->delete('lastactivity + 900 < '.time());
		
		if(empty($_COOKIE['onlineusernum'])){
			$onlinenum = syDB('sessions')->findCount();
			setcookie("onlineusernum", $onlinenum, time() + 300); 
		}else{
			$onlinenum = $_COOKIE['onlineusernum'];
		}
		if(empty($_COOKIE['totalnum'])){
			$totalnum = syDB('sysconfig')->find('name="totalnum"');
			setcookie("totalnum", $totalnum['sets'], time() + 300);
			$totalnum = $totalnum['sets'];
		}else{
			$totalnum = $_COOKIE['totalnum'];
		}
		$res['onlineusernum'] = $onlinenum;
